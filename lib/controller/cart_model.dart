...
class CartState {
  ...
  CartState({required this.id, required this.items})
  ...
  CartState copyWith({int? id, List<CartItem>? items}) {
    return CartState(id: id ?? this.id, items: items ?? this.items);
  }

  @override
  String toString() {
    return "CartState{total: $total, items: $items}";
  }
}
...
List<Product> products = [
  ...
  const Product(id: 2, name: "PREDATOR TRITON 500 SE", description: "Good", price: 1999.99, imageUrl: "https://images.acer.com/is/image/acer/Triton-500-SE-PT516-51s-FP-Backliton-01a-1?\$Visual-Filter-XL\$"),
